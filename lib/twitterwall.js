...
Twitterwall.prototype.flashTimeout = function(message, duration, callback){
  this.sendRequest('flash', {message: message, duration: duration}, function(err, data){
    if(!err && data && data.success) callback(null)
    else callback(err || (data && data.error) || 'could not flash message')
  })
}

Twitterwall.prototype.flash = function(message, callback){
  this.flashTimeout(message, 0, callback)
}
